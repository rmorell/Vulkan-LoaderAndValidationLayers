...
TEST_F(VkLayerTest, InUseDestroyedSignaled) {
    ...
    submit_info.commandBufferCount = 1;
    submit_info.pCommandBuffers = &m_commandBuffer->handle();
    const char *deleted_event_message[] = {"Cannot submit cmd buffer using deleted event 0x",
                                           "You are submitting command buffer 0x"};
    m_errorMonitor->SetDesiredFailureMsg(VK_DEBUG_REPORT_ERROR_BIT_EXT, deleted_event_message[0]);
    m_errorMonitor->SetDesiredFailureMsg(VK_DEBUG_REPORT_ERROR_BIT_EXT, deleted_event_message[1]);
    vkQueueSubmit(m_device->m_queue, 1, &submit_info, VK_NULL_HANDLE);
    m_errorMonitor->VerifyFound();
