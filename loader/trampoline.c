...
vkEnumeratePhysicalDevices(VkInstance instance, uint32_t *pPhysicalDeviceCount,
    ...
    if (NULL == temp_pd_array) {
        loader_log(inst, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                   ...
                   "physical device array of size %d", new_pd_count);
        res = VK_ERROR_OUT_OF_HOST_MEMORY;
        goto out;
    }
    ...
    if (NULL == new_phys_devs) {
        loader_log(inst, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                   ...
                   "device trampoline array of size %d", new_pd_count);
        res = VK_ERROR_OUT_OF_HOST_MEMORY;
        goto out;
    }
        ...
        if (NULL == new_phys_devs[new_idx]) {
            ...
            if (NULL == new_phys_devs[new_idx]) {
                loader_log(inst, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                           ...
                           "physical device trampoline object %d", new_idx);
...
LOADER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkCreateDevice(
    ...
    if (dev->activated_layer_list.list == NULL) {
        loader_log(inst, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                   ...
                   "list of size %d.", inst->activated_layer_list.capacity);
        res = VK_ERROR_OUT_OF_HOST_MEMORY;
        goto out;
    }
    ...
    if (inst->activated_layers_are_std_val) {
        ...
        if (!enabled_layers->list) {
            loader_log(inst, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                       ...
                       "layer list of size %d", enabled_layers->capacity);
