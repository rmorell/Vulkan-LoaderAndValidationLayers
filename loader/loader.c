...
loader_add_to_dev_ext_list(const struct loader_instance *inst,
    ...
    for (uint32_t i = 0; i < entry_count; i++) {
        ...
        if (ext_list->list[idx].entrypoints[i] == NULL) {
            ...
            loader_log(inst, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                       "loader_add_to_dev_ext_list: Failed to allocate space "
                       "for device extension entrypoint %d name", i);
            return VK_ERROR_OUT_OF_HOST_MEMORY;
        }
...
VKAPI_ATTR VkResult VKAPI_CALL terminator_CreateDevice(
    ...
    if (fpCreateDevice == NULL) {
        loader_log(icd_term->this_instance, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                   ...
                   "by ICD %s", icd_term->scanned_icd->lib_name);
        res = VK_ERROR_INITIALIZATION_FAILED;
        goto out;
    }
    ...
    if (res != VK_SUCCESS) {
        loader_log(icd_term->this_instance, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                   ...
                   "call", icd_term->scanned_icd->lib_name);
        goto out;
    }
        ...
        if (NULL != pPhysicalDevices) {
            ...
            if (NULL == icd_phys_devs[i].phys_devs) {
                loader_log(inst, VK_DEBUG_REPORT_ERROR_BIT_EXT, 0,
                           ...
                           "ICD %d with %d phys devs.", i,
                           icd_phys_devs[i].count);
                res = VK_ERROR_OUT_OF_HOST_MEMORY;
                goto out;
            }
